...
<script>
    document.addEventListener("DOMContentLoaded", function() {
        ...
        if (!overlay) {
            ...
            overlay.className = 'filter-overlay fixed inset-0 bg-black/60 z-[9990] hidden transition-opacity duration-300';
            document.body.appendChild(overlay);
        }

        // ĐẢM BẢO ẨN FORM KHI VỪA VÀO TRANG (QUAN TRỌNG)
        if (window.innerWidth < 1024) {
            filterSidebar.classList.remove('active-mobile');
        }
